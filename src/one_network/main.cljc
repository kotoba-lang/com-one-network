(ns one_network.main
  "Kotodama WASM entrypoint for the one_network clean-room actor (L4) — Clojure port.
  ...
  facts (`one_network.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "one_network")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"  :plural "tradeitems"  :id-prefix "onenetwo_tra"
    :fields [:gtin :description :brand]        :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"      :plural "events"      :id-prefix "onenetwo_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}          :refs {:itemId "TradeItem"}}
   {:entity "Document"   :plural "documents"   :id-prefix "onenetwo_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container"  :plural "containers"  :id-prefix "onenetwo_con"
    :fields [:bicCode :type :status]           :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"      :plural "parties"     :id-prefix "onenetwo_par"
    :fields [:gln :name :role]                 :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"   :plural "shipments"   :id-prefix "onenetwo_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"one_network.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "one_network-compat" :tier tier :entities entities} 200])
...
(defn start! [] :one_network-compat/ready)
